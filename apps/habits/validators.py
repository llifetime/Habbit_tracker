...
class HabitValidator:
    ...
    def __call__(self, data):
        ...
        if linked_habit and not linked_habit.is_pleasant:
            raise ValidationError(
                {"linked_habit": "В связанные привычки могут попадать только приятные привычки"}
            )
        ...
        if duration and duration > 120:
            raise ValidationError(
                {"duration_sec": "Время выполнения должно быть не больше 120 секунд"}
            )

        # 5. Нельзя выполнять привычку реже, чем 1 раз в 7 дней (проверка на минимальное значение)
